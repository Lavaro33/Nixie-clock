...
void uart_puts_P(const char *s)		// wysy≥a ≥aÒcuch z pamiÍci FLASH na UART
{
  register char c;
  while ((c = pgm_read_byte(s++))) uart_putc(c);			// dopÛki nie napotkasz 0 wysy≥aj znak
}
...
void uart_putlong(int value, int radix)	// wysy≥a na port szeregowy tekst
{
